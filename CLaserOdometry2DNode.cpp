...
CLaserOdometry2DNode::CLaserOdometry2DNode(): Node("CLaserOdometry2DNode")
{
  ...
  // {
  //   // init to 0
  //   RCLCPP_WARN(get_logger(), "is init to 0");
...
void CLaserOdometry2DNode::process()
{
  ...
  RCLCPP_WARN(get_logger(), "is_initialized: %s", rf2o_ref.is_initialized() ? "true" : "false");
  RCLCPP_WARN(get_logger(), "available_scan: %s", scan_available() ? "true" : "false");

  if( rf2o_ref.is_initialized() && scan_available() )
  {
  
    RCLCPP_WARN(get_logger(), "Yes...");
